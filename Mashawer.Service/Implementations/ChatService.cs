...
namespace Mashawer.Service.Implementations
{
    public class ChatService : IChatService
    {
        ...
        public ChatService(ApplicationDbContext context, IUnitOfWork unitOfWork, UserManager<ApplicationUser> userManager, IHubContext<ChatHub> hubContext, INotificationService notificationService)
        {
            _context = context;
        ...
        public async Task<IEnumerable<ConversationDTO>> GetAllConversationsAsync(string userId)
        {
            ...
            return conversations.Select(c => new ConversationDTO
            {
                ...
                UserName = c.SenderId == userId ? c.Receiver?.FullName ?? "Unknown" : c.Receiver?.FullName ?? "Unknown",
                //   UserImage = c.SenderId == userId ? c.Receiver?.ProfileImage : c.Sender?.ProfileImage,
        ...
        public async Task<IEnumerable<ConversationDTO>> GetAllConversationsAsync(string userId, string receiverName = null)
        {
            // استرجاع المحادثات التي أرسلتها أو استقبلتها
        ...
        public async Task<ChatMessage> SendMessageAsync(int conversationId, string senderId, string receiverId, string content)
        {
        ...
        public async Task<IEnumerable<MessageDto>> GetMessagesAsync(int conversationId)
        {
            ...
            return messages.Select(message => new MessageDto
            {
                ...
                //   SenderProfilePicture = users.TryGetValue(message.SenderId, out sender) ? sender.ProfileImage : null,
                ReceiverId = message.ReceiverId,
                RecipientName = users.TryGetValue(message.ReceiverId, out var receiver) ? receiver.UserName : "Unknown",
                //  RecipientProfilePicture = users.TryGetValue(message.ReceiverId, out receiver) ? receiver.ProfileImage : null,
        ...
        //{
        //    var sender = await _userManager.FindByIdAsync(senderId);
        //    return sender?.ProfileImage ?? "default_image_url";
        //}
        ...
        public async Task<string> MarkAllMessagesAsReadAsync(int conversationId, string userId)
        {
            var messages = await _unitOfWork.Messages.GetTableAsTracking()
           .Where(m => m.ConversationId == conversationId &&
                      m.ReceiverId == userId &&
                      !m.IsRead)
           .ToListAsync();
